...
static void append_and_clear(char * full, char * toAdd) {
	//combine respbuf with the temporary resp_buff 
        strcat(full, toAdd);
	//clear resp_buff.        
	memset(toAdd,0,MAX_LINE);
}

static void build_task_string(char * respbuf,struct task_struct * call_task) {
  char resp_buff[MAX_LINE]; // local (kernel) space for putting in lines to buffer.
  struct task_struct *parent = NULL;
  struct mm_struct *memory = NULL;
  struct rw_semaphore *sema = NULL;   
  ...
  //find neccesary relationships
  parent = (*call_task).real_parent; 
  memory = get_task_mm(call_task);
  sema = &(*memory).mmap_sem;
  
  //diable semaphores while working with memory
  down_read(sema);

  cur_pid = task_pid_nr(call_task);
  command  = kmalloc(MAX_LINE,GFP_ATOMIC);
  get_task_comm(command,call_task);
  par_pid = task_pid_nr(parent);
  state = (*call_task).state;
  flags = (*call_task).flags;
  priority = (*call_task).normal_prio;
                                ...
                                size_t count, loff_t *ppos)
{
  int rc;
  int siblings;
  char resp_buff[MAX_LINE];  //used to print warnings and other valid input
  ...
  //iterate through all the siblings and print them out
  
  //count siblings
  siblings = 0;  

  list_for_each_entry(i,&call_task->sibling,sibling) {
	//used to detect if I'm at head of list	
	pid_t test = task_pid_nr(i);
	
	//No i found, should check        
	if (i == NULL) {
		//list is empty, just a redundent check	
	}else if(test == 1) {
		//the first element of the list is current task, skip it.        	
		//do nothing
        } else if (siblings < MAX_SIBLINGS){
		//task is a sibling of current task. Add its fields to the response buffer		
		build_task_string(respbuf,i);
		siblings++;
	} else {
		siblings++;
	}
  ...
  //Next, test if Max siblings was reached and give a Message saying max buffer size was hit or no siblings were found
  if (siblings > MAX_SIBLINGS) {
	sprintf(resp_buff,"Number of siblings %i found. Only printed first %i siblings due to buffer size.\n\n",siblings,MAX_SIBLINGS);
	strcat(respbuf, resp_buff);
  } else if (siblings == 0) {
	//didnt find a sibling
	sprintf(resp_buff,"Caller has no siblings.\n\n");
	strcat(respbuf, resp_buff);
  }
    
  strcat(respbuf, "Calling Task has task info:\n\n");

  //finally print the current task. 
  build_task_string(respbuf,call_task);
