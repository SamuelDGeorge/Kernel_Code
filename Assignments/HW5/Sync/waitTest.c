...
void main (int argc, char* argv[])
{
  ...
  if(argc != 3) {
	fprintf(stdout, "Invalid Arguments, Only two int allowed\n");
	exit(-1);
  }

	fprintf(stdout, "Calling Wait in kernel on queue: %s Exclusive: %s\n", argv[1],argv[2]);

	
	sprintf(inputstring, "event_wait %s %s",argv[1],argv[2]);	
			
	
	do_syscall(inputstring);
		
	fprintf(stdout, "Wait on queue %s ended with code: %s\n", argv[1], resp_buf);			
