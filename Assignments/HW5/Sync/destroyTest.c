...
void main (int argc, char* argv[])
{
  ...
  if(argc != 2) {
	fprintf(stdout, "Invalid Arguments, Only one int allowed\n");
	exit(-1);
  }

	fprintf(stdout, "Calling destroy in kernel on queue: %s\n", argv[1]);

	
	sprintf(inputstring, "event_destroy %s",argv[1]);	
			
	
	do_syscall(inputstring);
		
	fprintf(stdout, "Destroy on queue %s ended with code: %s\n", argv[1], resp_buf);			
